...
ap.add_argument('--from_mesh',action='store_true')
...
if not from_mesh:
    ...
    try:
        ...
        array_recon = np.load(mesh)
    except:
        print('Loading binary mesh file; assuming fortran indexing')
        array_recon = np.fromfile(mesh,dtype=np.float32,sep='')
        array_recon = array_recon.reshape((N,N,N),order='F')
    

    mesh_recon = ArrayMesh(array_recon,BoxSize=box_size)
